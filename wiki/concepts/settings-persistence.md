...
Project registration lives in `~/.goto`. It is intentionally plain text so users can inspect and edit it directly. Recent project state lives in `~/.goto_recent`, capped at three paths. CLI sorting state lives in `~/.goto_config` as JSON encoded `GotoCLIConfig`.
