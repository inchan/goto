...
- 자동 업데이트: 인터랙티브 모드 진입 시 백그라운드로 새 버전을 확인하고(24시간마다, 결과는 `~/.goto_update_check`에 캐시되어 다음 실행 때 상단 한 줄로 안내), `goto --upgrade`로 최신 릴리스 DMG를 받아 `Install Goto.pkg`를 설치합니다(sudo 필요, 앱+CLI 모두 갱신). 로컬 개발 빌드(버전 `0.0.0`)에서는 확인을 건너뜁니다.
