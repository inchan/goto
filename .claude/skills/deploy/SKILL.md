---
<<<<<<<< HEAD:.claude/skills/deploy/SKILL.md
name: deploy
...
>>>>>>>> origin/main:.claude/skills/release/SKILL.md
...
- 실행 모드는 두 가지: **자동모드** / **수동모드** (Stage 0 시작 시 `AskUserQuestion`으로 1회 선택)
- 단계 도중 실패하면 모드와 무관하게 즉시 중단하고 사용자 판단을 기다린다
...
| 항목 | 자동모드 | 수동모드 |
|---|---|---|
| Stage 1 정리 범위 | 풀 스캔 자동 진행, 명백한 잔재만 정리 | 스캔 결과 후 `AskUserQuestion` 으로 범위 확정 |
| Stage 2 문서 패치 | 누락 발견 시 바로 적용 | 패치 미리보기 후 적용 여부 질문 |
| Stage 3 앱 설치 | CLI 단독 배포만, `./install.sh` 자동 호출 안 함 | 전체 앱 설치 여부 질문 |
| Stage 4 커밋 분할 | 자동 판단 (논리 단위 분리 권장, 1~2개) | 단일/분할 여부 질문 |
| Stage 4 push | 자동 수행 | 명시 승인 후 수행 |
| Stage 4 PR 생성 | 자동 수행 | 다음 액션 질문 |
| Stage 4 self-merge | 자동 squash + delete-branch=false | self-merge 여부 질문 |
| destructive 동작(`rm`, `git push --force`, `git reset --hard`) | 두 모드 모두 **항상 사용자 명시 승인 필요** | 두 모드 모두 **항상 사용자 명시 승인 필요** |

자동모드라도 위 표의 "두 모드 모두 항상 승인" 항목은 예외 없이 사용자 승인을 받는다.
